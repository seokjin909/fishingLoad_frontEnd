  ...
  return (
    <div className="relative overflow-x-auto w-[1200px] mt-8">
      <div className="flex items-center font-bold p-1 gap-2 mb-4">
        <div className="text-2xl border rounded-full p-2 bg-blue-500 text-white">
          <SlNote />
        </div>
      ...
      <table className="w-full text-md text-left text-gray-500 font-bold">
        ...
        <tbody>
          {data.map((item, index) => {
            return (
              <CommunityCard key={item.id} data={item} index={index + 1} />
            );
          })}
        </tbody>
      </table>
      <div
        className="float-right mt-4 bg-blue-400 py-2 px-4 rounded-md text-white cursor-pointer"
